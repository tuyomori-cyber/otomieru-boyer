...
縮小表示時にも再生線を滑らかに追従させるため、スペクトログラムは表示幅に合わせて時間方向の解析フレームを集約して描画する方針です。各描画列には対応する時間範囲の強度を集約し、画面外の細かな時間解像度による描画負荷を抑えます。
...
- `基音強調`：キー／スケールまたは任意の音を選び、倍音列を根拠に基音候補を表示上で強調する。指定外の音は減衰率を調整できる
