...
- 比較パターンは、最大10件の比較フェーズと現在位置を共有する軽量な原子的制御値として扱う。Originalではメモ出力を一時停止し、Notesでは原曲を無音化し、Mixでは両方を出力する。UIによる変更はパターン全体を一度に切り替えるため、callbackが部分更新を観測しない。レイヤーMute・音量の実行時設定は変更しない。
- 時間・音高の表示範囲はUI状態であり、再生位置や音声スレッドへ影響しない。
